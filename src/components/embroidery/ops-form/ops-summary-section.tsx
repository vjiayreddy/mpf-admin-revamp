...
export function OpsSummarySection({ detail }: OpsSummarySectionProps) {
  ...
  const hasImages = Boolean(fabric || reference)
  ...
  return (
    <section className="bg-card rounded-lg border p-5">
      ...
      >
          ...
          <div className="flex shrink-0 gap-2">
            {fabric ? (
              ...
              <img
                src={fabric}
                alt="Fabric"
                className="border-border size-20 rounded-sm border object-cover sm:size-24"
              />
            ) : null}
            {reference ? (
              // eslint-disable-next-line @next/next/no-img-element
        ...
        <dl className="grid min-w-0 flex-1 gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
          <div>
