...
export type QualityCheckHeaderProps = {
  title: string
...
export function QualityCheckHeader({
  title,
  subtitle,
  status,
  printDisabled,
    ...
    <div className="border-border flex shrink-0 flex-row items-start justify-between gap-3 border-b px-4 py-3">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {status ? (
            <span className={qualityCheckStatusChipClass(status)}>{status}</span>
          ) : null}
        </div>
        {subtitle ? (
