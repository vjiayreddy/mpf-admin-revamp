...
export type AppointmentsListPanelProps = {
  lockedUserId?: string
  /** Admin page shows full title; customer page uses a compact heading. */
  showPageChrome?: boolean
  /** Hide Name/Phone when the customer is already shown in the profile header. */
...
export function AppointmentsListPanel({
  ...
  const {
    ...
    lockedUserId ? { lockedUserId } : undefined
  )
  ...
  const columnDefs = useMemo(() => {
    const cols: ColDef<AppointmentListRow>[] = [
      {
        colId: "actions",
        headerName: "More",
        minWidth: 70,
    ...
    cols.push(
      ...
      },
      {
        ...
        headerName: "Time",
        minWidth: 100,
        valueGetter: (p) => formatTime(p.data?.appointmentSelectedTimestamp),
      },
      {
        colId: "studio",
        headerName: "Studio",
    ...
    return cols
  }, [openQuickView, openStatus, openEdit, hideCustomerColumns])
